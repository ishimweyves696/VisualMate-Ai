  ...
  return (
    <>
      ...
      <AnimatePresence>
        {isMobileOpen && (
          <motion.div 
            ...
            className="fixed inset-0 bg-zinc-900/40 backdrop-blur-sm z-[110] lg:hidden"
          />
        )}
      </AnimatePresence>
      ...
      <motion.aside 
        ...
        className={`fixed left-0 top-0 h-screen bg-white border-r border-zinc-200 z-[120] flex flex-col transition-all duration-300 ease-in-out lg:translate-x-0 ${isMobileOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}
      >
        {/* Logo Section */}
        ...
        <div className="p-4 border-t border-zinc-50 space-y-4">
            ...
            >
              <div className="flex items-center justify-between mb-3">
                <span className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest">
                  {isAuthenticated ? subscription.plan : 'GUEST'} Plan
                </span>
                <span className="text-[10px] font-bold text-zinc-900">
                  {isAuthenticated 
                    ? (subscription.plan === 'FREE' ? subscription.usedToday : subscription.usedThisMonth) 
                    : 'Preview Mode'} / {isAuthenticated 
                      ? (subscription.plan === 'FREE' ? PLANS.FREE.dailyQuota : PLANS.PRO.monthlyQuota) 
                      : '2 Free'}
                </span>
              </div>
              ...
              {(subscription.plan === 'FREE' || !isAuthenticated) && (
                <button 
                  onClick={onUpgrade}
                ...
                >
                  <Zap className="w-3 h-3 text-emerald-400" /> {isAuthenticated ? 'Upgrade to Pro' : 'Unlock Pro Access'}
                </button>
              )}
            </motion.div>
          ) : (
            <div className="flex flex-col items-center gap-4">
              ...
              {(subscription.plan === 'FREE' || !isAuthenticated) && (
                ...
                >
                  ...
                  <div className="absolute left-full ml-2 px-2 py-1 bg-zinc-900 text-white text-[10px] rounded opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity whitespace-nowrap z-50">
                    {isAuthenticated ? 'Upgrade' : 'Unlock Pro'}
                  </div>
                </button>
              )}
            </div>
          ...
          <button 
            ...
            className="w-full hidden lg:flex items-center justify-center py-2 text-zinc-400 hover:text-zinc-600 transition-colors"
          >
